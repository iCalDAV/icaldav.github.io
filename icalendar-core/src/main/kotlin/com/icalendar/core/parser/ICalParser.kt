...
class ICalParser {
    ...
    fun parseAllEvents(icalData: String): ParseResult<List<ICalEvent>> {
        return try {
            println("[ICalParser] parseAllEvents: Starting, input size=${icalData.length}")
            val startTime = System.currentTimeMillis()

            println("[ICalParser] parseAllEvents: Unfolding...")
            val unfolded = unfoldICalData(icalData)
            println("[ICalParser] parseAllEvents: Unfolded in ${System.currentTimeMillis() - startTime}ms")

            println("[ICalParser] parseAllEvents: Creating CalendarBuilder...")
            val builder = CalendarBuilder()
            println("[ICalParser] parseAllEvents: CalendarBuilder created in ${System.currentTimeMillis() - startTime}ms")

            println("[ICalParser] parseAllEvents: Calling build()...")
            val calendar = builder.build(StringReader(unfolded))
            println("[ICalParser] parseAllEvents: build() completed in ${System.currentTimeMillis() - startTime}ms")

            println("[ICalParser] parseAllEvents: Getting VEVENT components...")
            val events = calendar.getComponents<VEvent>(Component.VEVENT)
                .mapNotNull { vevent ->
                    parseVEvent(vevent).getOrNull()
                }
            println("[ICalParser] parseAllEvents: Parsed ${events.size} events in ${System.currentTimeMillis() - startTime}ms total")

            ParseResult.success(events)
        } catch (e: Exception) {
            println("[ICalParser] parseAllEvents: EXCEPTION: ${e.javaClass.simpleName}: ${e.message}")
