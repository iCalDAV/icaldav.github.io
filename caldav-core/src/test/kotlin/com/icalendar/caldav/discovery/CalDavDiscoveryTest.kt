...
class CalDavDiscoveryTest {
    ...
    fun setup() {
        ...
        webDavClient = WebDavClient(
            httpClient = WebDavClient.defaultHttpClient(),
    ...
    inner class ICloudCompatibilityTests {
        ...
        fun `handles caldav dot icloud dot com redirect`() {
            // iCloud often redirects from caldav.icloud.com
            server.enqueue(
                MockResponse()
                    .setResponseCode(301)
                    .setHeader("Location", "https://p123-caldav.icloud.com/")
            )

            // Discovery should handle redirect appropriately
            val result = discovery.discoverPrincipal(serverUrl("/"))
            assertNotNull(result)
        }
    }
